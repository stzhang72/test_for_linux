...
DEFINE_KFIFO(mydemo_fifo,char,8192);
...
static struct inode * myfs_get_inode(struct super_block * sb, int mode, dev_t dev)
{
	...
	if(inode)
	{
		inode->i_mode = mode | S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP | S_IROTH | S_IWOTH;
		...
		//@i_mtime：最后修改时间
		//@i_ctime：最后修改inode时间
		// 修改这里，替换为 __i_atime、__i_mtime 和 __i_ctime
	...
	printk("Setting inode with UID = %d, GID = %d\n", inode->i_uid, inode->i_gid);
...
static ssize_t myfs_file_write(struct file *file, const char __user *buf, size_t count, loff_t *ppos)
{
    ...
    int ret;
    struct inode *inode = file->f_inode;  // 获取文件的 inode
    size_t new_size;

    // 将数据从用户空间传递到内核空间的 FIFO 队列
    ret = kfifo_from_user(&mydemo_fifo, buf, count, &actual_write);
    if (ret)
        return -EIO;  // 如果数据传输失败，返回错误

    printk("%s: actual_write=%d, ppos=%lld\n", __func__, actual_write, *ppos);

    // 更新文件偏移量
    *ppos += actual_write;  // 更新偏移量，以便下一次写入从正确的位置开始

    // 计算新的文件大小（基于当前偏移量 + 写入的数据量）
    new_size = *ppos;

    // 如果新文件大小超过了当前文件大小，更新 inode 中的 i_size
    if (new_size > inode->i_size) {
        inode->i_size = new_size;
        inode->__i_mtime = current_time(inode);  // 更新文件的修改时间
        printk("File size increased to %zu bytes\n", inode->i_size);

        // 标记 inode 为脏数据，确保文件元数据被保存到磁盘
